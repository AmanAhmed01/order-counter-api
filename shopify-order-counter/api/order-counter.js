export default async function handler(req, res) {
  ...
  try {
    ...
    // Filter Orders: Paid or Pending only & tag "daraz" OR "accessories" not allowed
    const validOrders = data.orders.filter(order => {
      const isValidStatus = ["paid", "pending"].includes(order.financial_status);

      const tags = order.tags ? order.tags.toLowerCase() : "";
      const hasDarazTag = tags.includes("daraz");
      const hasAccessoriesTag = tags.includes("accessories");

      return isValidStatus && !hasDarazTag && !hasAccessoriesTag;
    });
